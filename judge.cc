...
void make_registrations(std::vector<Participant> &participants) {
  participants.push_back(Participant(new alvaro_garcia(), "alvaro_garcia"));
  participants.push_back(Participant(new arthur_tadier(), "arthur_tadier"));
  participants.push_back(
      Participant(new juan_diego_castro(), "juan_diego_castro"));
  participants.push_back(Participant(new luis_izaguirre(), "luis_izaguirre"));
  participants.push_back(Participant(new melanie_cortez(), "melanie_cortez"));
  participants.push_back(Participant(new jerimy_sandoval(), "jerimy_sandoval"));

  // Descalificado: segmentation fault.
  // participants.push_back(Participant(new cesar_perales(), "cesar_perales"));
  // participants.push_back(Participant(new melisa_rivera(), "melisa_rivera"));
  // participants.push_back(Participant(new roger_huauya(), "roger_huauya"));
}
...
int main() {
  ...
  for (int i = 0; i < participants.size(); ++i) {
    std::cout << std::setw(20) << participants[i].name << "  ";
    if (participants[i].disqualified) {
      std::cout << "disqualified due to invalid move" << std::endl;
    } else {
      std::cout << "won: " << participants[i].won
                << "  lost: " << participants[i].lost
                << "  drawn: " << participants[i].drawn << std::endl;
    }
  }
